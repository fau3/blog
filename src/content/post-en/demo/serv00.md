...
## 1. Keeping the account alive

- Since a Serv00 account will be reclaimed if you do not connect by SSH or log into the web panel for three months, you need to log in from time to time to avoid losing the account. Here I use a Cloudflare Worker to perform scheduled logins and send the login results through a Telegram bot.
- Worker code:
...
- Then trigger it once to see whether the login succeeds

![](https://roim-picx-9nr.pages.dev/rest/jl1QrLK.png)

If Telegram successfully pushes the message, then it is working.

![](https://roim-picx-9nr.pages.dev/rest/siiRrLK.png)

## 2. Keeping the process alive

- Serv00 may occasionally clear running processes and the cron jobs we configured. Re-adding cron jobs every time is annoying, so I use a script to add them with one click.
- Worker:
...
```txt
The password for accessing your Worker panel
```
...
- Create a KV variable named `CRON_RESULTS`
- Go back to the Worker page you created earlier and bind the KV namespace

![](https://roim-picx-9nr.pages.dev/rest/xpzrrLK.png)

- Visit your Worker site, enter the password, and click `Run Script`
